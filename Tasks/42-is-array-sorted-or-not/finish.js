...
const arraySortInfo = (inputArray) => {
  if (inputArray.some((element) => typeof element !== "number")) {       //метод some - возвращает логическое значение 
    return "Некоторые элементы не являются числами";
  }

  if (
    inputArray.every((element, index) =>                                 // метод every должен вернуть true, если все вызовы колбек функции вернули true, т.е нам необходимо с этой функции возвращать правдивое значение
