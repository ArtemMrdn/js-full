...
const photosGallery = (title, dimensions, date) => {   // Функции которые ввозвращаются другой функцией имеют доступ ко всем параметрам родительской функции, даже если она уже  
  return {                                             // закончила свою работу ии благодаря такому поведению JS мы можем получать доступ к параметрам например функции photosGallery,
    title,                                             // как в данном примере, просто читая значения параметров title либо date это поведение называется ЗАМЫКАНИЕМ
    date,                                       
    [dimensions]: true,                                //динамически добавленное новое свойство
    info() {
